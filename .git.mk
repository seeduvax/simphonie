...
.PHONY: check-format
check-format:
	@cd $(PRJROOT); .git/hooks/$(APPNAME)/apply-format --staged

## - format: format locally your git staged contents you made and that you are about to commit
.PHONY: format
format:
	@cd $(PRJROOT); .git/hooks/$(APPNAME)/apply-format --staged -i && git status

## - format-cached: format git staged contents you made and that you are about to commit
.PHONY: format-cached
format-cached:
	@cd $(PRJROOT); .git/hooks/$(APPNAME)/apply-format --cached -i && git status

# insert the hook as an early dependency to trigger its installation as soon as possible
$(PRJROOT)/app.cfg: $(PRJROOT)/.git/hooks/pre-commit.d/code-style $(PRJROOT)/.git/hooks/$(APPNAME)/apply-format
